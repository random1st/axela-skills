...
- **Worth knowing before you install**: `scripts/with_server.py` starts a development server through a
  shell. That is what the script is for, it is upstream's own code, and we read it line by line and accepted
  it — but it is the one plugin here that runs a shell command on your machine by design, so anyone
  uncomfortable with that should skip it.
